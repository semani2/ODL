...
//Adding observation into observations and temperature table
	
	float x=Float.parseFloat(request.getParameter("temp_amt"));
	...
	int i = st1.executeUpdate("INSERT INTO observations(ot_id,pid,ob_dto,ob_dtr) VALUES(10,"+id+",'"+dt_o+"','"+dt_r+"')");
	if(i>0)
	{
	ResultSet rs=st3.executeQuery("SELECT ob_id from observations where ob_dtr='"+dt_r+"' AND pid="+id+"");
	rs.next();
	int j= st3.executeUpdate("INSERT INTO temperature(ot_id,t_amt,ob_id) VALUES (10,"+x+","+rs.getInt(1)+")");
	if(j>0)
	{
	...
	{
		st4.executeUpdate("delete from observations where ob_id="+rs.getInt(1)+"");
		response.sendRedirect("newobservation.jsp?error=Observation could not be recorded !");
	}
	}
	else
	{
